...
class Zend_Service_ReCaptcha_MailHide extends Zend_Service_ReCaptcha
{
     ...
     */
    public function __construct($publicKey = null, $privateKey = null, $email = null, $params = null)
    {
        ...
        /* If params is a Zend_Config object we want to convert it to an array so we can merge it with the default params */
        ...
        parent::__construct($publicKey, $privateKey, null, $params);
    ...
    public function getHtml($email = null)
    {
        ...
        $html = htmlentities($this->_emailLocalPart) . '<a href="' . htmlentities($url) . '" onclick="window.open(\'' . htmlentities($url) . '\', \'\', \'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=0,width=' . $this->_params['popupWidth'] . ',height=' . $this->_params['popupHeight'] . '\'); return false;" title="' . $this->_params['linkTitle'] . '">' . $this->_params['linkHiddenText'] . '</a>@' . htmlentities($this->_emailDomainPart);
